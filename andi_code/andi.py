...
class andi_datasets():
    ...
    def create_dataset(self, T, N, exponents, models,
        ...
        ### added a small change here to allow for N_save with different values for each model (transpose multiply)
        N_save = (np.ones_like(n_per_class).T*N_save).T
    ...
    def andi_dataset(self, N = 1000, max_T = 1000, min_T = 10,
        ...
        for dim in dimensions:             
            ...
            dataset = self.create_dataset(T = max_T, N = n_per_class, exponents = exponents, 
                                           ...
                                           load_trajectories = load_trajectories, save_trajectories = save_trajectories, N_save = N_save,
    ...
    #this is a copy of andi_dataset but modified to always return labels for all of exponent, model and noise regardless of task
    def andi_dataset_superlabeled(self, N = 1000, max_T = 1000, min_T = 10,
                           tasks = [1, 2, 3],
                           dimensions = [1, 2, 3],
                           load_dataset = False, save_dataset = False, path_datasets = '',
                           load_trajectories = False, save_trajectories = False, path_trajectories = 'datasets/',
        ...
        Return:
            ...
            :X1 (list of three lists):
                - Trajectories corresponding to Task 1. 
            :Y1 (list of three lists):
                - Labels corresponding to Task 1, modified to be model, exponent, noise labels
            :X2 (list of three lists):
                - Trajectories corresponding to Task 2. 
            :Y2 (list of three lists):
                - Labels corresponding to Task 2, modified to be model, exponent, noise labels
            :X3 (list of three lists):
                - Trajectories corresponding to Task 3. 
            :Y3 (list of three lists):
                - Labels corresponding to Task 3, modified to be model, exponent, noise labels      '''
        ...
        for dim in dimensions:             
            ...
            if 1 in tasks:         
                ...
                for traj in dataset_1[:N, :]:             
                    # Cutting trajectories
                    cut_T = np.random.randint(min_T, max_T) 
                    traj_cut = self.cut_trajectory(traj[3:], cut_T, dim=dim).tolist()                         
                    # Saving dataset
                    X1[dim-1].append(traj_cut)
                    Y1[dim-1].append(np.around(traj[0:3], 2))
                    if save_dataset:                        
                        self.save_row(np.append(dim, traj_cut), task1)
                        self.save_row(np.append(dim, np.around(traj[0:3], 2)), ref1)
            ...
            if 2 in tasks:   
                ...
                for traj in dataset_2[:N, :]:             
                    ...
                    # Saving dataset   
                    X2[dim-1].append(traj_cut)
                    Y2[dim-1].append(np.around(traj[0:3], 2))    
                    if save_dataset:
                        self.save_row(np.append(dim, traj_cut), task2)
                        self.save_row(np.around(traj[0:3], 2), ref2)   
           
                     
            # Task 3 - Segmentated trajectories
            if 3 in tasks:  
                raise("Super Dataset does not support task 3!") 
                        
                        
        return X1, Y1, X2, Y2, X3, Y3
    
    #andi dataset but for only one model, therefore only task 1!
    def andi_dataset_onemodel(self, N = 1000, max_T = 1000, min_T = 10, tasks = [1],
                           dimensions = [1, 2, 3],
                           load_dataset = False, save_dataset = False, path_datasets = '',
                           load_trajectories = False, save_trajectories = False, path_trajectories = 'datasets/',
                           N_save = 1000, t_save = 1000, model = 0, superset = False):  
        ''' Creates a dataset similar to the one given by in the ANDI challenge. 
        Check the webpage of the challenge for more details. The default values
        are similar to the ones used to generate the available dataset.
        Arguments:  
        ...
        if model == 0 or model == 1: #attm and ctrw
            exponents = np.arange(0.05, 1.01, 0.05)
        elif model == 3: #lw
            exponents = np.arange(1.05, 2.01, 0.05)
        elif model == 2: #fbm not ballistic!
            exponents = np.arange(0.05, 1.999, 0.05)
        else: #sbm
            exponents = np.arange(0.05, 2.01, 0.05)
